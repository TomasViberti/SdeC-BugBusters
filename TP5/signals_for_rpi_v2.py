import socket
import time
import math

SOCKET_PATH = "/tmp/qtest.sock"
PERIODO  = 1.0
MUESTRAS = 20
dt       = PERIODO / MUESTRAS

GPSET0 = 0x3F20001C
GPCLR0 = 0x3F200028

GPIO4  = 4
GPIO17 = 17

def enviar_comando(sock, cmd):
    sock.sendall((cmd + "\n").encode())
    return sock.recv(1024).decode().strip()

def set_gpio_digital(sock, pin, valor):
    reg = GPSET0 if valor else GPCLR0
    cmd = f"writel 0x{reg:08x} 0x{(1 << pin):08x}"
    enviar_comando(sock, cmd)

def senoidal_digital(t):
    """Senoidal convertida a 0/1 con umbral 0.5"""
    valor = (math.sin(2 * math.pi * t / PERIODO) + 1) / 2
    return 1 if valor > 0.5 else 0

def cuadrada(t):
    return 1 if (t % PERIODO) < (PERIODO / 2) else 0

with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as sock:
    sock.connect(SOCKET_PATH)
    print("Conectado a qtest.")
    t = 0.0
    try:
        while True:
            val_sen = senoidal_digital(t)
            val_cua = cuadrada(t)

            set_gpio_digital(sock, GPIO4,  val_sen)
            set_gpio_digital(sock, GPIO17, val_cua)

            print(f"t={t:.2f}s | GPIO4={val_sen} | GPIO17={val_cua}")
            time.sleep(dt)
            t += dt
    except KeyboardInterrupt:
        print("Señales detenidas.")